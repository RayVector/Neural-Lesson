...
for e in range(epochs):
    ...
    for input_stat, correct_predict in train2:
        network2.training(np.array(input_stat), correct_predict)
        inputs_.append(np.array(input_stat))
        correct_predictions.append(np.array(correct_predict))

    train_loss = mse(network2.predict(np.array(inputs_).T), np.array(correct_predictions))
    sys.stdout.write("\rProgress: {}, Training loss: {}".format(str(100 * e / float(epochs))[:4], str(train_loss)[:5]))
...
for input_stat, correct_predict in train2:
    print("For input: {} the prediction is: {}, expected: {}".format(
        str(input_stat),
        str(int(network2.predict(np.array(input_stat)) > .5)),
        str(int(correct_predict == 1))
    ))
...
for input_stat, correct_predict in train2:
    print("For input: {} the prediction is: {}, expected: {}".format(
        str(input_stat),
        str(network2.predict(np.array(input_stat))),
        str(int(correct_predict == 1))
    ))